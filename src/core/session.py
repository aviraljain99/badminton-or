...
class Session:
    def __init__(
        self, players: list[Player], courts: int, rounds: int, min_games: int, max_games: int
    ):
            ...
            for r in range(self.ROUNDS):
                for c in range(self.COURTS):
                    for t in range(TEAM_SIZE):
                        self.variables[(p, r, c, t)] = self.model.new_int_var(
                            0, 1, f"x_{p}_{r}_{c}_{t}"
                        )
                ...
                for r in range(self.ROUNDS):
                    for c in range(self.COURTS):
                        for t in range(TEAM_SIZE):
                            ...
                            self.variables[(p1, p2, r, c, t)] = self.model.new_int_var(
                                0, 1, f"x_{p1}x{p2}_{r}_{c}_{t}"
                            )
                            ...
                            self.model.add(
                                (2 * self.variables[(p1, p2, r, c, t)])
                                < (
                                    self.variables[(p1, r, c, t)]
                                    + self.variables[(p2, r, c, t)]
                                    + 1
                                )
                            )
                            self.model.add(
                                (2 * self.variables[(p1, p2, r, c, t)]) + 2
                                > (self.variables[(p1, r, c, t)] + self.variables[(p2, r, c, t)])
                            )
    ...
    def __game_rule_constraints(self):
        ...
        for r in range(self.ROUNDS):
            for c in range(self.COURTS):
                for t in range(TEAM_SIZE):
                    self.model.add(
                        sum(self.variables[(p, r, c, t)] for p in range(self.TOTAL_PLAYERS))
                        == TEAM_SIZE
                    )
                ...
                if player_data[p]["member"]:
                    for p1 in range(self.TOTAL_PLAYERS):
                        if not player_data[p1]["member"] and p != p1:
                            self.model.add(
                                sum(
                                    self.variables[(p, r1, c, t)]
                                    for r1 in range(r + 1)
                                    for c in range(self.COURTS)
                                    for t in range(TEAM_SIZE)
                                )
                                >= sum(
                                    self.variables[(p1, r1, c, t)]
                                    for r1 in range(r + 1)
                                    for c in range(self.COURTS)
                                    for t in range(TEAM_SIZE)
                                )
