...
def get_players_in_team(
    round: int, court: int, team_id: int, variables: dict[tuple, IntVar], solver: CpSolver
) -> list[str, str]:
    team = []
...
def get_players_on_break(round: int, variables: dict[tuple, IntVar], solver: CpSolver) -> list[str]:
    ...
    for player_id, player_name in enumerate(player_data):
        if all(
            solver.value(variables[(player_id, round, court, team)]) == 0
...
def get_games_in_round(
    round: int, solver: CpSolver, variables: dict[tuple, IntVar]
) -> list[list[list[str]]]:
...
def get_round_as_md(
    games_in_round: list[list[list[str]]], round: int, players_on_break: list[str]
) -> str:
    md = ""
    ...
    for court_id, (teams, players_on_break) in enumerate(
        zip(games_in_round, players_on_break_chunks)
    ):
        team_a, team_b = teams
